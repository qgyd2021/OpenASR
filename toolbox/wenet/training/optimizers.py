...
from toolbox.wenet.common import Params, Registrable
...
class Optimizer(Registrable):
    ...
    @classmethod
    def from_params(cls, params: Params, **extra) -> torch.optim.Optimizer:
        model_parameters: List = extra['model_parameters']
        ...
        subclass: torch.optim.Optimizer = Optimizer.by_name(optimizer)

        # If the optimizer subclass has a from_params, use it.
        if hasattr(subclass, 'from_params'):
            return subclass.from_params(params=params, model_parameters=parameter_groups)
        else:
            return subclass(parameter_groups, **params_as_dict)
